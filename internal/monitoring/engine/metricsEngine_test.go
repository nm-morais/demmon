...
func TestEvaluateNumericExpression(t *testing.T) {
	...
	err = tsdb.AddMetric("test", 15)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
	clock.Add(1 * time.Second)
	err = tsdb.AddMetric("test", 17)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
	err = tsdb.AddMetric("test", 14)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
func TestEvaluateNumericExpression2(t *testing.T) {
	...
	tsdb.AddMetric("test", toAdd)
	clock.Add(1 * time.Second)
	tsdb.AddMetric("test", 12)
	clock.Add(1 * time.Second)
	tsdb.AddMetric("test", 10)
...
func TestEvaluateNumericExpression4(t *testing.T) {
	...
	tsdb.AddMetric("test", 3)
	clock.Add(1 * time.Second)
	tsdb.AddMetric("test", 1)
	clock.Add(1 * time.Second)
