...
do
  ...
  *)
  ...
  node=$(nextnode $i)
  echo "oarsh $node 'docker run --rm -v $SWARM_VOL:/code/logs -d -t --cpus=$cpu}} --cap-add=NET_ADMIN --net $net --ip $ip --name $name -h $name $DOCKER_IMAGE $i'"
  oarsh -n $node "docker run --rm -v $SWARM_VOL:/code/logs -d -t --cpus=$cpu --cap-add=NET_ADMIN --net $net --ip $ip --name $name -h $name $DOCKER_IMAGE $i"
