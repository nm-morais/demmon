...
for node in $@; do
  if [ $node != $host ]; then
    oarsh $node "mkdir $SWARM_VOL_DIR > /dev/null; docker volume rm $SWARM_VOL > /dev/null; docker volume create $SWARM_VOL --opt type=none --opt device=$SWARM_VOL_DIR --opt o=bind"
